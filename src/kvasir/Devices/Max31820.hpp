...
struct Max31820 {
    ...
    static_assert(OneWire::BufferSize >= readout_packet_size,
    ...
    void handler() {
        ...
        case State::wait_for_conversion:
            {
                if(currentTime > waitTime_ && OneWire::acquire()) {
                    OneWire::send_receive(currentTime,
                                          std::array{0xCC_b, 0xBE_b},
                                          readout_packet_size);
                ...
                case OS::succeeded:
                    {
                        ...
                        if(OneWire::crc(buffer.begin(), buffer.end()) != 0_b) {
                            ...
                            auto const raw = static_cast<std::int16_t>(
                              static_cast<std::uint16_t>(static_cast<std::uint16_t>(buffer[1])
                                                         << 8_u16)
